...
?>
  ...
  <div class="">
    <a href="shopping.php?id=<?php echo $r['id_product'] ?>">
      ...
      <form action="<?php echo $_SERVER["PHP_SELF"] ?>" class="d-align" method="post">
    <h6><?php echo $r['price'] ?>£<input type="hidden" name="product_id" value="<?=$r['id_product']?>">
    
    <input type="submit"   name="add" class="btn btn-secondary btn-sm  " value="Add card" > </h6></form>
  </div>
<?php }
...
function filter_data()
{
    ...
    $.ajax({
        url:"filter-data/filter-data_mini-pc.php",
        method:"POST",
